...
type PredefinedRateKey = 'weekday' | 'weekend' | 'partial_day' | 'holiday' | 'telemedicine';
...
export function termsToRates(terms: {
  ...
  if (terms.weekday_rate) entries.push({ type: 'weekday', label: 'Weekday Rate', amount: terms.weekday_rate, kind: k('weekday') });
  if (terms.weekend_rate) entries.push({ type: 'weekend', label: 'Weekend Rate', amount: terms.weekend_rate, kind: k('weekend') });
  if (terms.partial_day_rate) entries.push({ type: 'partial_day', label: 'Partial Day Rate', amount: terms.partial_day_rate, kind: k('partial_day') });
  if (terms.holiday_rate) entries.push({ type: 'holiday', label: 'Holiday Rate', amount: terms.holiday_rate, kind: k('holiday') });
  if (terms.telemedicine_rate) entries.push({ type: 'telemedicine', label: 'Telemedicine Rate', amount: terms.telemedicine_rate, kind: k('telemedicine') });
  if (terms.custom_rates) {
    terms.custom_rates.forEach(cr => entries.push({ type: 'custom', label: cr.label, amount: cr.amount, kind: cr.kind || 'flat' }));
  }
  return entries;
}
...
export function ratesToTermsFields(rates: RateEntry[]) {
  ...
  for (const r of rates) {
    switch (r.type) {
      ...
      case 'custom': fields.custom_rates.push({ label: r.label, amount: r.amount, kind: r.kind }); break;
    }
  }
  return fields;
}
          ...
          <div className="inline-flex rounded-md border border-border overflow-hidden h-9 shrink-0" role="group">
            {(['flat', 'hourly'] as RateKind[]).map(k => (
              <button
                key={k}
                type="button"
                onClick={() => { handleUpdateRate(i, { kind: k }); onSave?.(rates.map((r, idx) => idx === i ? { ...r, kind: k } : r)); }}
                className={cn(
