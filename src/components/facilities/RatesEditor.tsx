...
export function termsToRates(terms: {
  ...
  if (terms.weekday_rate) entries.push({ type: 'weekday', label: 'Weekday Rate', amount: terms.weekday_rate });
  if (terms.weekend_rate) entries.push({ type: 'weekend', label: 'Weekend Rate', amount: terms.weekend_rate });
  if (terms.partial_day_rate) entries.push({ type: 'partial_day', label: 'Partial Day Rate', amount: terms.partial_day_rate });
  if (terms.holiday_rate) entries.push({ type: 'holiday', label: 'Holiday Rate', amount: terms.holiday_rate });
  if (terms.telemedicine_rate) entries.push({ type: 'telemedicine', label: 'Telemedicine Rate', amount: terms.telemedicine_rate });
  if (terms.custom_rates) {
    terms.custom_rates.forEach(cr => entries.push({ type: 'custom', label: cr.label, amount: cr.amount }));
  }
  return entries;
}
...
export function ratesToTermsFields(rates: RateEntry[]) {
  const fields = {
    ...
    holiday_rate: 0,
    telemedicine_rate: 0,
    custom_rates: [] as Array<{ label: string; amount: number }>,
  };
  for (const r of rates) {
    switch (r.type) {
      case 'weekday': fields.weekday_rate = r.amount; break;
      case 'weekend': fields.weekend_rate = r.amount; break;
      case 'partial_day': fields.partial_day_rate = r.amount; break;
      case 'holiday': fields.holiday_rate = r.amount; break;
      case 'telemedicine': fields.telemedicine_rate = r.amount; break;
      case 'custom': fields.custom_rates.push({ label: r.label, amount: r.amount }); break;
    }
  }
  return fields;
}
...
export function RatesEditor({ rates, onChange, onSave, showCard = true, compact = false }: RatesEditorProps) {
  ...
  const handleUpdateRate = useCallback((index: number, field: 'label' | 'amount', value: string | number) => {
      ...
      {rates.map((rate, i) => (
        <div key={i} className="flex items-center gap-2">
          {rate.type === 'custom' ? (
            ...
            <span className="flex-1 text-sm font-medium">{rate.label}</span>
          )}
