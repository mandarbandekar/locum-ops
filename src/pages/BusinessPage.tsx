...
import TaxCopilotPage from '@/pages/TaxCopilotPage';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 sm:gap-3 flex-wrap">
        ...
        <button
          onClick={() => setSearchParams({ tab: 'tax-copilot' }, { replace: true })}
          className={`primary-tab-btn ${activeTab === 'tax-copilot' ? 'primary-tab-btn--active' : 'primary-tab-btn--inactive'}`}
        >
          <Landmark className="h-4 w-4 sm:h-5 sm:w-5" />
          <span className="text-xs sm:text-sm">Tax Copilot</span>
        </button>
      </div>

      {activeTab === 'reports' && <ReportsPage />}
      {activeTab === 'tax-copilot' && <TaxCopilotPage />}
    </div>
  );
}
