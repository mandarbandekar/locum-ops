...
import { CREDENTIAL_TYPE_LABELS, computeCredentialStatus } from '@/lib/credentialTypes';
...
import { Plus, Search, LayoutGrid, List, MoreHorizontal, Pencil, Trash2, Copy, RefreshCw, FileDown, Archive } from 'lucide-react';
...
export default function CredentialsPage() {
  ...
  const handleDuplicate = async (cred: Credential) => {
    const { id, user_id, created_at, updated_at, ...rest } = cred;
    // addCredential is accessed through the dialog — for now just open dialog with prefilled data
    setEditingCredential({ ...cred, id: '', custom_title: `${cred.custom_title} (Copy)` });
    setDialogOpen(true);
  };
