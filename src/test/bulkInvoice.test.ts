...
const makeShift = (id: string, facilityId: string, daysAgo: number): Shift => {
  const d = new Date();
...
describe('Bulk Invoice Helpers', () => {
  ...
  it('includes all shifts for the selected facility', () => {
    const shifts = [
      makeShift('s1', 'c1', 5),
      makeShift('s2', 'c1', 3),
      makeShift('s3', 'c2', 4),
  ...
  it('calculates bulk draft total correctly', () => {
    const shifts = [
      { ...makeShift('s1', 'c1', 5), rate_applied: 850 },
      { ...makeShift('s2', 'c1', 3), rate_applied: 1100 },
    ];
    const { eligible } = getEligibleShiftsForBulkInvoice(shifts, [], [], 'c1', periodStart, periodEnd);
  ...
  it('preserves shift traceability via shift_id', () => {
    const shifts = [makeShift('s1', 'c1', 5)];
    const { eligible } = getEligibleShiftsForBulkInvoice(shifts, [], [], 'c1', periodStart, periodEnd);
