-- Founder Dashboard RPC: returns one row per beta tester with activation metrics.
-- Admin-only: enforced server-side via auth.email() against an internal allowlist.
...
DECLARE
  caller_email text;
  admin_emails text[] := ARRAY['mandar@locum-ops.com'];
BEGIN
