...
DROP FUNCTION public.get_founder_overview();

CREATE OR REPLACE FUNCTION public.get_founder_overview()
 RETURNS TABLE(
   user_id uuid, email text, display_name text,
   signed_up_at timestamptz, last_sign_in_at timestamptz,
   clinic_count integer, shift_count integer, invoice_count integer,
   downloaded_invoice_count integer, credential_count integer, expense_count integer,
   last_activity_at timestamptz, activation_status text,
   last_device text, desktop_sign_ins integer, mobile_sign_ins integer, tablet_sign_ins integer
 )
 LANGUAGE plpgsql
 SECURITY DEFINER
