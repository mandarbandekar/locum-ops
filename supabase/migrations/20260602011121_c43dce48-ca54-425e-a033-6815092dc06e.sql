...
BEGIN
  ...
  shift_stats AS (
    SELECT s.user_id, COUNT(*)::int AS c, GREATEST(MAX(s.created_at), MAX(s.updated_at)) AS last_at FROM public.shifts s GROUP BY s.user_id
  ),
  invoice_stats AS (
    SELECT i.user_id, COUNT(*)::int AS c, GREATEST(MAX(i.created_at), MAX(i.updated_at)) AS last_at FROM public.invoices i GROUP BY i.user_id
  ),
  download_stats AS (
    SELECT d.user_id, COALESCE(SUM(d.download_count), 0)::int AS c, MAX(d.last_downloaded_at) AS last_at FROM public.invoice_pdf_downloads d GROUP BY d.user_id
  ),
  signin_event_stats AS (
    SELECT ev.user_id, MAX(ev.created_at) AS last_at FROM public.user_sign_in_events ev GROUP BY ev.user_id
  ),
  credential_stats AS (
  ...
  SELECT
    ...
    GREATEST(
      ...
      COALESCE(ds.last_at, 'epoch'::timestamptz),
      COALESCE(ses.last_at, 'epoch'::timestamptz),
      COALESCE(u.last_sign_in_at, 'epoch'::timestamptz)
    ),
    CASE
      WHEN u.last_sign_in_at IS NULL AND ses.last_at IS NULL THEN 'never'
      WHEN GREATEST(
        COALESCE(fs.last_at, 'epoch'::timestamptz),
        COALESCE(ss.last_at, 'epoch'::timestamptz),
        COALESCE(invs.last_at, 'epoch'::timestamptz),
        ...
        COALESCE(ses.last_at, 'epoch'::timestamptz),
        COALESCE(u.last_sign_in_at, 'epoch'::timestamptz)
      ) >= (now() - INTERVAL '14 days') THEN 'active'
      ELSE 'dormant'
    END,
    ld.device_type,
  ...
  LEFT JOIN signin_event_stats ses ON ses.user_id = u.id
