CREATE OR REPLACE FUNCTION public.get_user_last_activity(_user_id uuid, _since timestamptz DEFAULT NULL)
RETURNS TABLE(window_start timestamptz, last_activity_at timestamptz, last_table text, last_device text)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
AS $$
DECLARE
  _start timestamptz := _since;
BEGIN
  SELECT ev.created_at INTO last_activity_at
  ...
  WITH touched AS (
    ...
    UNION ALL
    SELECT 'credential_documents', GREATEST(cd.uploaded_at, cd.updated_at)
      FROM public.credential_documents cd WHERE cd.user_id = _user_id AND (cd.uploaded_at >= _start OR cd.updated_at >= _start)
    UNION ALL
  ...
  SELECT
    _start,
    (SELECT MAX(ts) FROM touched),
    (SELECT t FROM touched ORDER BY ts DESC NULLS LAST LIMIT 1),
    (SELECT ev.device FROM public.user_sign_in_events ev WHERE ev.user_id = _user_id ORDER BY ev.created_at DESC LIMIT 1);
END;
$$;